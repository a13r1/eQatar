...
public class UpdateItemViewController implements Initializable {
	...
	private void submitBtnAction() throws Exception {
		...
		if (priceStr.isBlank() || brand.isBlank() || color.isBlank()) {
		...
		try {
			...
			Alert alert = new Alert(AlertType.CONFIRMATION, "Please provide numeric value price", ButtonType.OK);
			alert.showAndWait();
			return;
		}
		...
		if (type.equals("Camera")) {
			...
			GUIApp.getEQatarSystem().modifyProperties(camera);
			GUIApp.getCurrentUser().modifyProperties(camera);
		} else if (type.equals("Smartphone")) {
			...
			GUIApp.getEQatarSystem().modifyProperties(smartphone);
			GUIApp.getCurrentUser().modifyProperties(smartphone);
		} else if (type.equals("Video Game")) {
			...
			GUIApp.getEQatarSystem().modifyProperties(videoGame);
			GUIApp.getCurrentUser().modifyProperties(videoGame);
		}
